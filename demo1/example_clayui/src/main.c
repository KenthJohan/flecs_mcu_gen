...
SDL_AppResult SDL_AppInit(void **appstate, int argc, char *argv[])
{
    ...
    *appstate = state;
	...
	state->rendererData.fonts_count = 1;
    state->rendererData.fonts = SDL_calloc(state->rendererData.fonts_count, sizeof(TTF_Font *));
    if (!state->rendererData.fonts) {
        SDL_LogError(SDL_LOG_CATEGORY_ERROR, "Failed to allocate memory for the font array: %s", SDL_GetError());
        return SDL_APP_FAILURE;
    }

    TTF_Font *font = TTF_OpenFont("/usr/share/fonts/truetype/ubuntu/Ubuntu-L.ttf", 24);
    if (!font) {
...
void SDL_AppQuit(void *appstate, SDL_AppResult result)
{
    ...
    if (state) {
        ...
        if (state->rendererData.fonts) {
            for(size_t i = 0; i < state->rendererData.fonts_count; i++) {
